"""Canonical inventory identity and display helpers."""
...
def listing_signature_fields(parsed: Any, raw_sender: str = "", group_name: str = "") -> dict[str, str]:
    ...
    return {
        ...
        "location": _first_text(data.get("micro_market"), data.get("location_raw"), data.get("building_name"), data.get("landmark_name")),
        "building": _text(data.get("building_name")),
        "landmark": _text(data.get("landmark_name")),
        "broker": _first_text(data.get("broker_phone"), data.get("profile_name"), data.get("broker_name"), raw_sender),
    }
...
def listing_label(parsed: Any) -> str:
    data = _mapping(parsed)
    return _first_text(data.get("micro_market"), data.get("location_raw"), data.get("building_name"), data.get("landmark_name")).title()
