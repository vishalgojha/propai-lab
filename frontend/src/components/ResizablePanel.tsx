...
export default function ResizablePanel({
    ...
    return (
      <div className={`relative flex flex-col ${className}`} style={{ width: 48, minWidth: 48 }}>
        <button
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-md bg-[#111820] border border-[rgba(255,255,255,0.08)] text-[#64748b] hover:text-white hover:border-[#3EE88A]/40 transition-all flex items-center justify-center"
          title="Expand panel"
        >
  ...
  return (
    ...
    >
      ...
      <button
        onClick={toggleCollapse}
        className="absolute top-2 right-3 z-30 w-5 h-5 rounded bg-[#111820] border border-[rgba(255,255,255,0.06)] text-[#4a5568] hover:text-white hover:border-[#3EE88A]/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100"
      ...
      {showPresets && (
        <div
          className="absolute top-10 right-3 z-40 bg-[#111820] border border-[rgba(255,255,255,0.1)] rounded-lg shadow-xl overflow-hidden"
          ...
          {presets.map((preset) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-[11px] hover:bg-[rgba(255,255,255,0.05)] transition-colors flex items-center justify-between gap-6 ${
                width === preset.width ? "text-[#3EE88A]" : "text-[#94a3b8]"
              }`}
            >
              <span>{preset.label}</span>
              <span className="text-[10px] text-[#4a5568] font-mono">{preset.width}px</span>
            </button>
          ))}
        </div>
      )}
