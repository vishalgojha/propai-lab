...
function chipClass(tone?: string) {
  ...
  return "bg-white/5 text-[#cbd5e1] border-[rgba(255,255,255,0.08)]";
}

function frame(children: ReactNode, className = "") {
  return <div className={`rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0d1117] p-4 ${className}`}>{children}</div>;
}
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {block.subtitle && <div className="text-xs text-[#64748b] mt-1">{block.subtitle}</div>}
        </div>
        {block.note && <div className="text-[10px] text-[#64748b]">{block.note}</div>}
      </div>
      {(block.body || block.description || block.summary) && (
        compact?.bullets && compact.bullets.length > 0 ? (
          <div className="grid gap-2 sm:grid-cols-2">
            {compact.bullets.map((bullet) => (
              <div key={bullet} className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#0b1016] px-3 py-2 text-sm text-[#e2e8f0]">
                {bullet}
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm text-[#cbd5e1] leading-6 whitespace-pre-wrap">{narrative}</p>
        )
      )}
...
function CompactCard({
  ...
  return (
    <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0b1016] p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-semibold text-white truncate">{title || "—"}</div>
          {subtitle && <div className="text-[11px] text-[#64748b] mt-1">{subtitle}</div>}
        </div>
      </div>
      {meta && meta.length > 0 && <div className="mt-2 flex flex-wrap gap-2 text-[10px] text-[#94a3b8]">{meta.map((item) => <span key={item} className="rounded-full border border-[rgba(255,255,255,0.06)] px-2 py-0.5">{item}</span>)}</div>}
      {body && <p className="mt-2 text-xs text-[#cbd5e1] leading-5 whitespace-pre-wrap">{body}</p>}
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {(block.subtitle || block.note) && <div className="text-xs text-[#64748b] mt-1">{block.subtitle || block.note}</div>}
        </div>
        {block.body && <div className="text-[10px] text-[#94a3b8]">{block.body}</div>}
      </div>
      {items.length === 0 ? (
        <div className="text-sm text-[#64748b]">{block.description || "No results."}</div>
      ) : (
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          ...
          {items.length > 10 && <div className="text-[10px] text-[#64748b]">+{items.length - 10} more shown in the current response</div>}
        </div>
      )}
    </div>,
  );
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-[rgba(255,255,255,0.06)]">
              {columns.map((column) => (
                <th key={column} className="pb-2 pr-4 font-semibold text-[#94a3b8] uppercase tracking-wider text-[10px]">{column}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((row, idx) => (
              <tr key={idx} className="border-b border-[rgba(255,255,255,0.04)]">
                {columns.map((column, colIdx) => (
                  <td key={`${idx}-${column}`} className="py-2 pr-4 text-[#e2e8f0] align-top">
        ...
        {items.map((item, index) => (
          <div key={item.id || index} className="flex gap-3">
            ...
            <div className="flex-1 pb-1">
              ...
              {item.when && <div className="text-[10px] text-[#64748b] mt-0.5">{item.when}</div>}
              {item.body && <div className="text-xs text-[#cbd5e1] mt-1 whitespace-pre-wrap">{item.body}</div>}
            </div>
          </div>
        ))}
      </div>
  ...
  return frame(
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {items.map((item, index) => (
          <div key={item.id || index} className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#0b1016] p-3">
            <div className="text-[10px] text-[#64748b] flex flex-wrap gap-x-2 gap-y-1">
            ...
            <div className="text-sm text-[#e2e8f0] mt-1 whitespace-pre-wrap">{item.message || item.body || item.text || "—"}</div>
          </div>
        ))}
      </div>
    </div>,
          ...
          return (
            <div key={item.label || index}>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-[#cbd5e1]">{item.label || item.name || `Item ${index + 1}`}</span>
  ...
  return frame(
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-[10px] text-[#64748b]">Map preview</div>
      </div>
      <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#091018] p-4 min-h-40">
        <div className="grid grid-cols-2 gap-2">
          {items.slice(0, 6).map((item, index) => (
            <div key={item.id || index} className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-white/5 p-2">
              <div className="text-sm text-white">{item.label || item.name || item.title || `Pin ${index + 1}`}</div>
              <div className="text-[10px] text-[#94a3b8] mt-1">{item.subtitle || item.location || item.address || "Listing pin"}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {channels.map((channel) => {
          ...
          return (
            <div key={channel.id} className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0b1016] p-3">
              <div className="text-[10px] uppercase tracking-wider text-[#64748b] mb-2">{channel.label}</div>
              <div className="text-xs text-[#e2e8f0] whitespace-pre-wrap leading-5">{lines.join("\n\n") || "—"}</div>
            </div>
          );
        })}
      </div>
  ...
  if (block.type === "error_state") {
    return frame(
      <div className="space-y-2">
        ...
        <div className="text-sm text-[#cbd5e1] whitespace-pre-wrap">{block.body || block.description || block.summary || "The assistant could not complete that request."}</div>
      </div>,
    );
  }
  if (block.type === "empty_state") {
    return frame(
      <div className="space-y-2">
        ...
        <div className="text-sm text-[#94a3b8] whitespace-pre-wrap">{block.body || block.description || block.summary || "Try a different query."}</div>
      </div>,
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-[#64748b] mt-1">Structured broker results rendered by PropAI</div>
        </div>
        <div className="text-[10px] text-[#64748b]">{filteredBlocks.length} block{filteredBlocks.length === 1 ? "" : "s"}</div>
      </div>

      {statusSteps.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {statusSteps.map((step, index) => (
            <span key={`${step}-${index}`} className="rounded-full border border-[rgba(255,255,255,0.08)] bg-white/5 px-2.5 py-1 text-[10px] text-[#cbd5e1]">
              {step}
            </span>
          ))}
        </div>
      ...
      {sources.length > 0 && (
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-full border px-3 py-1 text-[10px] transition-colors ${!hasSourceFilter ? "border-blue-500/30 bg-blue-500/10 text-blue-200" : "border-[rgba(255,255,255,0.08)] bg-white/5 text-[#cbd5e1] hover:text-white"}`}
          >
            All sources
          </button>
          {sources.map((source) => (
            <button
              ...
              className={`rounded-full border px-3 py-1 text-[10px] transition-colors ${
                selectedSource === source
                  ...
                  : "border-[rgba(255,255,255,0.08)] bg-white/5 text-[#cbd5e1] hover:text-white"
              }`}
            >
              {sourceLabel(source)}
            </button>
      ...
      {hasSourceFilter && (
        <div className="text-[10px] text-[#64748b]">
          Filtering to source: <span className="text-[#cbd5e1]">{selectedSource}</span>
        </div>
      )}

      {filteredBlocks.length === 0 ? (
        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0d1117] p-4 text-sm text-[#94a3b8]">
      ...
      <div className="flex flex-wrap items-center gap-2 text-[10px] text-[#64748b]">
        ...
        {sources.map((source) => (
          <span key={source} className="rounded-full border border-[rgba(255,255,255,0.06)] bg-white/5 px-2 py-0.5">{sourceLabel(source)}</span>
        ))}
      </div>
    </div>
  );
