...
type DatabaseApproval = { token: string; operation: string; summary: string; table?: string; row_id?: string; function_name?: string; values?: Record<string, unknown>; arguments?: Record<string, unknown>; resource?: string };
...
function DatabaseApprovalCard({ approval }: { approval: DatabaseApproval }) {
  ...
  const isCoolify = approval.operation === "coolify_restart" || approval.operation === "coolify_redeploy";
  const label = isCoolify
    ? `${approval.operation === "coolify_restart" ? "Restart" : "Redeploy"} ${approval.resource || ""}`.trim()
    : approval.operation === "run_function" ? `Run ${approval.function_name || "database function"}` : approval.operation.replace("_", " ");
  ...
  return <div className="mt-3 rounded-lg border border-amber-400/35 bg-amber-400/8 p-3 text-xs text-[var(--text-secondary)]">
    ...
    <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-[var(--text-muted)]"><span>{label}</span>{!isCoolify && approval.table && <span>Table: <code>{approval.table}</code></span>}{!isCoolify && approval.row_id && <span>Row: <code>{approval.row_id}</code></span>}</div>
