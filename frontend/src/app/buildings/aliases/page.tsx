...
export default function AliasSuggestionsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-white">Building Alias Engine</h1>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="bg-zinc-900 border border-white/10 rounded-xl p-4">
            <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Total Discovered</div>
            <div className="text-2xl font-bold text-white">{stats.total_suggestions}</div>
          </div>
          <div className="bg-zinc-900 border border-white/10 rounded-xl p-4">
          ...
          <div className="bg-zinc-900 border border-white/10 rounded-xl p-4">
            <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Aliases in KB</div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-zinc-500">{suggestions.length} suggestions</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <div className="text-zinc-500 text-sm">No {filter} suggestions</div>
          {filter === "pending" && (
            <button
              onClick={handleDiscover}
              className="mt-3 px-4 py-2 bg-zinc-800 border border-white/10 rounded-lg text-sm text-zinc-500 hover:text-white"
            >
              Discover Aliases
            </button>
          )}
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <div>
                      <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Canonical</div>
                      <div className="text-sm font-bold text-white">{s.canonical}</div>
                    </div>
                    <span className="text-zinc-500 text-lg">↔</span>
                    <div>
                      <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Alias</div>
                      <div className="text-sm font-bold text-white">{s.alias}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-zinc-500">Source: {s.source}</span>
                  </div>
                  ...
                  {s.reasons && s.reasons.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {s.reasons.map((r: string, i: number) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 bg-zinc-800 border border-white/10 rounded text-[10px] text-zinc-400"
                        >
                          {r}
                        </span>
                      ))}
                ...
                {filter === "pending" && (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-[#3EE88A] text-black rounded-lg text-xs font-bold hover:bg-[#2dd67a]"
                    >
                      Approve
                    </button>
                    <button
                      onClick={() => handleReview(s.id, false)}
                      className="px-3 py-1.5 bg-zinc-800 border border-white/10 rounded-lg text-xs font-semibold text-zinc-500 hover:text-white"
                    >
                      Reject
                    </button>
                  </div>
