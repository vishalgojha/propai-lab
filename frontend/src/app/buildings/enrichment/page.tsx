...
export default function BuildingEnrichmentDashboardPage() {
  ...
  if (loading) {
    return <div className="text-[#64748b]">Loading enrichment dashboard...</div>;
  }
    ...
    <div className="space-y-6">
              ...
              <div key={i} className="bg-[#0a0f14] border border-[rgba(255,255,255,0.06)] rounded-lg p-3">
                ...
                <div className="mt-2 space-y-1 text-xs">
                  <div className="flex justify-between">
                    <span className="text-[#64748b]">Jobs</span>
                    <span>{p.jobs}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#64748b]">Enriched</span>
      ...
      <div>
              ...
              <button
                ...
                className={`text-xs px-2 py-1 rounded ${
                  jobFilter === status
                    ...
                    : "bg-[rgba(255,255,255,0.06)] text-[#64748b] hover:bg-[rgba(255,255,255,0.1)]"
                }`}
              >
                {status || "All"}
              </button>
        ...
        {filteredJobs.length === 0 ? (
          <div className="text-center py-8 text-[#64748b]">No jobs found</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  <th className="text-left px-2.5 py-2 border-b border-[rgba(255,255,255,0.1)] text-[11px] text-[#64748b] uppercase">Provider</th>
                  <th className="text-left px-2.5 py-2 border-b border-[rgba(255,255,255,0.1)] text-[11px] text-[#64748b] uppercase">Status</th>
                  <th className="text-right px-2.5 py-2 border-b border-[rgba(255,255,255,0.1)] text-[11px] text-[#64748b] uppercase">Priority</th>
                  <th className="text-left px-2.5 py-2 border-b border-[rgba(255,255,255,0.1)] text-[11px] text-[#64748b] uppercase">Created</th>
                  <th className="text-left px-2.5 py-2 border-b border-[rgba(255,255,255,0.1)] text-[11px] text-[#64748b] uppercase">Error</th>
                </tr>
              </thead>
              <tbody>
                {filteredJobs.slice(0, 50).map((j: any) => (
                  ...
                  >
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)] font-mono text-xs">{j.id}</td>
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)]">
                      <div className="font-semibold text-sm">{j.canonical_name}</div>
                      <div className="text-[#64748b] text-xs font-mono">{j.building_code}</div>
                    </td>
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)] capitalize">{j.provider}</td>
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)]">
                      <StatusBadge status={j.status} />
                    </td>
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)] text-right">{j.priority}</td>
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)] text-xs text-[#64748b]">{j.created_at}</td>
                    <td className="px-2.5 py-2 border-b border-[rgba(255,255,255,0.06)] text-xs text-[#ff6b35] max-w-[200px] truncate">
                      {j.error || "—"}
                    </td>
                  </tr>
                ))}
              ...
              <div key={i} className="bg-[#0a0f14] border border-[rgba(255,255,255,0.06)] rounded p-3 text-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[#64748b] ml-2 font-mono text-xs">{h.building_code}</span>
                  </div>
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    pending: "bg-[rgba(255,255,255,0.1)] text-[#64748b]",
  ...
  return (
    <span className={`text-xs px-1.5 py-0.5 rounded ${colors[status] || "bg-[rgba(255,255,255,0.1)] text-[#64748b]"}`}>
      {status}
    </span>
  );
}
