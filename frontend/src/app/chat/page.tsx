    ...
    <div className="max-w-4xl mx-auto px-4 lg:px-0">
      ...
      <div className="flex gap-1 mb-4 lg:mb-6 border-b border-white/10 pb-2 overflow-x-auto">
        <button onClick={() => setTab("chat")}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium ${tab === "chat" ? "bg-blue-600 text-white" : "text-zinc-400 hover:text-white"}`}
        >
          💬 AI Chat
        </button>
        <button onClick={() => setTab("review")}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium ${tab === "review" ? "bg-blue-600 text-white" : "text-zinc-400 hover:text-white"}`}
        >
        ...
        <>
          ...
          {memory && usage && (
            <div className="mb-6 grid grid-cols-2 gap-3">
              <div className="bg-zinc-900 border border-white/10 rounded-xl p-3">
                <div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">AI Memory</div>
              ...
              <div className="bg-zinc-900 border border-white/10 rounded-xl p-3">
                <div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Today&apos;s AI Usage</div>
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
          ...
          {selected.size > 0 && filter === "pending" && (
            <div className="mb-4 flex items-center gap-3 bg-blue-600/10 border border-blue-500/20 rounded-xl px-4 py-2.5">
              ...
              <button
                ...
                disabled={actionId === -1}
                className="px-3 py-1.5 text-xs font-medium text-zinc-500 hover:text-white border border-white/10 rounded-lg"
              >
                Ignore Selected
              </button>
              <button
                onClick={() => setSelected(new Set())}
                className="px-3 py-1.5 text-xs font-medium text-zinc-500 hover:text-white ml-auto"
              >
                Clear
              </button>
            </div>
          ...
          {rejectModal && !rejectModal.fromBatch && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setRejectModal(null)}>
              <div className="bg-zinc-900 border border-white/10 rounded-xl p-5 w-96" onClick={(e) => e.stopPropagation()}>
                <h3 className="text-sm font-semibold mb-3">Reject Suggestion #{rejectModal.id}</h3>
                <p className="text-xs text-zinc-400 mb-3">Why are you rejecting this suggestion? (optional)</p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {["Temporary / project name", "Not a real building", "Duplicate of existing", "Wrong locality", "Incorrect price", "Not useful"].map((r) => (
                    <button key={r} onClick={() => setRejectReason(r)}
                      className={`text-xs px-2.5 py-1 rounded-lg border ${rejectReason === r ? "bg-red-500/20 border-red-500/40 text-red-300" : "border-white/10 text-zinc-400 hover:text-white"}`}
                    >{r}</button>
                  ))}
                </div>
                <textarea
                  ...
                  className="w-full bg-[#161b22] border border-white/10 rounded-lg p-2 text-xs text-white placeholder-[#64748b] resize-none h-16 mb-3"
                />
                <div className="flex justify-end gap-2">
                  <button onClick={() => setRejectModal(null)} className="px-3 py-1.5 text-xs font-medium text-zinc-400 border border-white/10 rounded-lg">Cancel</button>
          ...
          {batchRejectOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setBatchRejectOpen(false)}>
              <div className="bg-zinc-900 border border-white/10 rounded-xl p-5 w-96" onClick={(e) => e.stopPropagation()}>
                ...
                <p className="text-xs text-zinc-400 mb-3">Why are you rejecting these suggestions? (optional — all get same reason)</p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {["Temporary / project name", "Not a real building", "Duplicate of existing", "Wrong locality", "Incorrect price", "Not useful"].map((r) => (
                    <button key={r} onClick={() => setRejectReason(r)}
                      className={`text-xs px-2.5 py-1 rounded-lg border ${rejectReason === r ? "bg-red-500/20 border-red-500/40 text-red-300" : "border-white/10 text-zinc-400 hover:text-white"}`}
                ...
                />
                <div className="flex justify-end gap-2">
                  <button onClick={() => setBatchRejectOpen(false)} className="px-3 py-1.5 text-xs font-medium text-zinc-400 border border-white/10 rounded-lg">Cancel</button>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Description */}
                        {s.description && (
                          <p className="text-xs text-zinc-400 whitespace-pre-wrap mb-2">{s.description}</p>
                        )}
                        ...
                        {impact.length > 0 && s.status === "pending" && (
                          <div className="mb-1">
                            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mr-2">Impact</span>
                            {impact.map((line, i) => (
                        ...
                        {s.status !== "pending" && s.source_data && (
                          <details className="mt-1">
                            <summary className="text-[10px] text-zinc-500 cursor-pointer hover:text-white">Raw data</summary>
                            <pre className="text-[10px] text-zinc-500 mt-1 bg-[#161b22] p-2 rounded overflow-x-auto">{JSON.stringify(s.source_data, null, 1)}</pre>
                          </details>
                        )}
                      </div>
                    </div>
