...
function tableExportMarkdown(textParts: Array<{ text: string }>) {
  const body = textParts.map((p) => p.text).join("\n\n");
  return `${body}\n\n---\n\n_Generated by **PropAI**_`;
}
...
export default function ChatPage() {
  ...
  // Copy the full answer (summary + GFM table) as portable markdown with the
  // required PropAI attribution footer, matching the rendering policy.
  const copyAnswer = useCallback(async (textParts: Array<{ text: string }>) => {
    try {
      await navigator.clipboard.writeText(tableExportMarkdown(textParts));
      setCopiedTable(true);
  ...
  // Download the full answer as a portable markdown file with attribution.
  const exportAnswer = useCallback((textParts: Array<{ text: string }>) => {
    const markdown = tableExportMarkdown(textParts);
    const blob = new Blob([markdown], { type: "text/markdown;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `propai-listings-${new Date().toISOString().slice(0, 10)}.md`;
    anchor.click();
                            ...
                            {hasTable && (
                              <div className="flex items-center gap-2">
                                ...
                                <div className="ml-auto flex items-center gap-1.5">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => exportAnswer(textParts)}
                                    className="rounded-md border border-white/10 px-2 py-1 text-[10px] font-medium uppercase tracking-[0.14em] text-zinc-400 hover:text-zinc-200 hover:border-white/25"
                                  >
                                    Export
                                  </button>
