...
export default function ClientDetailPage() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#070b0e] text-[#e2e8f0] p-6 text-center text-xs text-[#64748b] py-12">Loading client...</div>;
  }

  if (!client) {
    return (
      <div className="min-h-screen bg-[#070b0e] text-[#e2e8f0] p-6">
        <div className="max-w-3xl mx-auto text-center py-12">
          <div className="text-sm text-[#64748b]">Client not found.</div>
  ...
  return (
    ...
    >
      ...
      {client.requirements && client.requirements.length > 0 && (
        <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#0d1117] overflow-hidden">
          <button
            onClick={() => setShowRequirements(!showRequirements)}
            className="w-full flex items-center justify-between px-5 py-3 text-sm font-bold text-[#e2e8f0] hover:bg-[rgba(255,255,255,0.02)] transition-colors"
          >
          ...
          {showRequirements && (
            <div className="px-5 pb-4 space-y-2">
              {client.requirements.map((req) => (
                <div key={req.id} className="bg-[#111820] rounded-lg p-3 text-xs">
                  ...
                  <div className="text-[#64748b] space-y-0.5">
                    ...
                    {req.notes && <div className="text-[#94a3b8] mt-1">{req.notes}</div>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {showCandidates && (
            <div className="px-5 pb-4 space-y-2">
              {client.candidates.map((c) => (
                <div key={c.id} className="bg-[#111820] rounded-lg p-3 text-xs">
                  <div className="flex items-center gap-2 mb-1">
      ...
      <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#0d1117] overflow-hidden">
        ...
        {showMessages && (
          <div className="px-5 pb-4">
            {msgLoading ? (
              <div className="text-center text-xs text-[#64748b] py-8">Loading messages...</div>
            ) : messages.length === 0 ? (
              <div className="text-center py-8">
                <MessageSquare className="w-8 h-8 mx-auto text-[#64748b] mb-2" strokeWidth={1.5} />
                <div className="text-xs text-[#64748b]">No messages found for this client.</div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className={`flex items-center gap-1.5 mt-1.5 ${msg.direction === "outbound" ? "justify-end" : "justify-start"}`}>
                        <span className={`text-[9px] ${msg.direction === "outbound" ? "text-green-300" : "text-[#64748b]"}`}>
