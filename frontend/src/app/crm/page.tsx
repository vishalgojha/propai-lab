...
import { Armchair, Building2, CalendarDays, MapPin, Ruler, Search, ShieldCheck, Trash2, Upload } from "lucide-react";
...
export default function PrivateCrmPage() {
  ...
  async function remove(id: number) {
    if (!window.confirm("Remove this private inventory record?")) return;
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)]">
      <div className="mx-auto max-w-6xl px-5 py-10 lg:px-8">
        <div className="flex flex-col gap-6 border-b border-[var(--border-subtle)] pb-7 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm text-[var(--text-secondary)]">Your inventory stays inside this workspace. It is not published to the market and is not used for Auto Match.</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => inputRef.current?.click()} disabled={busy} className="inline-flex items-center gap-2 rounded-lg bg-[var(--signal-dim)] px-4 py-2.5 text-sm font-semibold text-[var(--parchment)] transition hover:bg-[var(--signal)] disabled:opacity-60"><Upload size={16} />{busy ? "Importing…" : "Import CSV"}</button>
          </div>
        </div>
