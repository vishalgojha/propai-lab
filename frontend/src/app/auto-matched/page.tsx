...
export default function AutoMatchedPage() {
  ...
  const load = async () => { setBusy(true); try { setData(await getAutoMatched()); } catch (e) { setMessage(e instanceof Error ? e.message : "Unable to load matches"); } finally { setBusy(false); } };
  useEffect(() => { void load(); }, []);
  const run = async () => { setBusy(true); setMessage("Running a bounded sample for this workspace…"); try { const result = await runAutoMatched({ limit_requirements: 50, distinct_cap: 5 }); setMessage(`${result.match_rows_written ?? 0} match rows written. Refreshing…`); await load(); } catch (e) { setMessage(e instanceof Error ? e.message : "Matching failed"); } finally { setBusy(false); } };
  return <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
    ...
    {message && <p className="mb-5 rounded-xl border border-border-subtle bg-surface-raised px-4 py-3 text-sm text-text-muted">{message}</p>}
    {!data ? <div className="rounded-2xl border border-border-subtle bg-surface-raised p-8 text-sm text-text-muted">Loading matches…</div> : data.requirements.length === 0 ? <div className="rounded-2xl border border-border-subtle bg-surface-raised p-10 text-center"><p className="font-medium text-text-primary">No qualified matches yet</p><p className="mt-1 text-sm text-text-muted">Run the bounded sample after the matcher has current listings and requirements.</p></div> : <>
