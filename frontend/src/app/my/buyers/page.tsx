    ...
    <div className="max-w-5xl">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#64748b]">Buyer requirements observed across WhatsApp groups.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="mt-6 rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#0d1117] p-8 text-center">
          <div className="text-sm font-semibold text-[#e2e8f0]">No market requirements observed yet.</div>
          <div className="mx-auto mt-2 max-w-xl text-sm text-[#64748b]">
            Buyer requirements from WhatsApp conversations will appear here as they are observed.
          </div>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 text-[10px] text-[#64748b]">
                    <span className="badge badge-purple text-[8px]">MARKET REQUIREMENT</span>
                    <span>Observed via</span>
                    <span className="text-[#cbd5e1] font-semibold">{req.raw_group || req.broker_name || "WhatsApp"}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-[#64748b] mt-1">
                    <User className="w-3 h-3" strokeWidth={1.5} />
                    <span className="text-[#cbd5e1] font-semibold">{req.broker_name || "Unknown Broker"}</span>
                    {req.broker_phone && (
                      <>
                        <span>•</span>
                    ...
                    {req.raw_timestamp && (
                      <>
                        <span>•</span>
                        <span>{new Date(req.raw_timestamp).toLocaleDateString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                      </>
                    )}
                  </div>
