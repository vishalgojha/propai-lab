  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-white">Knowledge Trainer</h1>
      ...
      {stats && (
        <div className="grid grid-cols-8 gap-3 mb-6">
          {[
            { label: "Total", value: stats.total, bg: "bg-white/5", text: "text-zinc-400" },
            ...
            { label: "Ignored", value: stats.ignored || 0, bg: "bg-white/5", text: "text-zinc-500" },
          ].map(s => (
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 mb-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-1 rounded text-xs bg-[rgba(255,255,255,0.06)] text-zinc-500 hover:bg-[rgba(255,255,255,0.10)] transition-colors"
            >
              Ignore All
            </button>
          </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${
              filter === f
                ...
                : "bg-white/5 text-zinc-400 hover:bg-[rgba(255,255,255,0.08)] hover:text-white"
            }`}
          >
            {f || "All"}
          </button>
      ...
      <div className="flex gap-1 mb-4 border-b border-white/10">
        {[
          ...
          { id: "noise" as const, label: "Parser Noise", count: parserNoise.length, color: "text-zinc-500" },
          { id: "resolved" as const, label: "Resolved", count: resolvedTerms.length, color: "text-zinc-500" },
        ].map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
          ...
          >
            {t.label}
            <span className={`ml-1.5 text-xs ${tab === t.id ? t.color : "text-zinc-500"}`}>({t.count})</span>
            {tab === t.id && (
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-white mb-1">No terms found</h3>
          ...
          {tab === "resolved" && resolvedTerms.length > 0 && (
            <div>
              ...
              {resolvedTerms.map(term => (
                <div key={term.id} className="bg-zinc-900 border border-white/5 rounded-lg p-3 mb-1.5 opacity-60">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="font-medium text-zinc-400">{term.term}</span>
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        ...
                        term.status === "other" ? "bg-white/5 text-zinc-400" :
                        "bg-white/5 text-zinc-500"
                      }`}>
                        {term.status}
                      </span>
                      <span className="text-xs text-zinc-500">{term.frequency}x</span>
                      {term.notes && (
                        <span className="text-xs text-zinc-500 italic">— {term.notes}</span>
                      )}
                    </div>
...
function TermCard({
  ...
  return (
    <div className={`bg-zinc-900 border rounded-lg p-4 mb-2 transition-colors ${
      isNoise
        ? "border-dashed border-white/10 hover:border-[rgba(255,255,255,0.10)]"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1.5">
            <span className="font-semibold text-white text-base">{term.term}</span>
            <span className="text-xs bg-white/5 text-zinc-500 px-2 py-0.5 rounded-full">
              {term.frequency}x
            </span>
            {isNoise && (
              <span className="text-xs bg-white/5 text-zinc-500 px-2 py-0.5 rounded-full">
          ...
          {message && (
            <div className="text-xs text-zinc-400 bg-black rounded p-2 mb-2 font-mono leading-relaxed border border-white/5">
          ...
          <div className="mb-2">
            <input
              ...
              className="w-full text-xs bg-black border border-white/10 rounded px-2.5 py-1.5 text-zinc-400 placeholder-[#4a5568] outline-none focus:border-blue-500/40 transition-colors"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            <button
              ...
              disabled={resolving}
              className="px-2.5 py-1 rounded text-xs bg-[rgba(255,255,255,0.06)] text-zinc-500 hover:bg-[rgba(255,255,255,0.10)] disabled:opacity-50 transition-colors"
