...
function age(value: unknown) { const seconds = Math.max(0, Number(value || 0)); if (!seconds) return "No actionable messages"; if (seconds < 60) return `${Math.round(seconds)} sec old`; if (seconds < 3600) return `${Math.floor(seconds / 60)} min old`; if (seconds < 86400) return `${Math.floor(seconds / 3600)} hr ${Math.floor((seconds % 3600) / 60)} min old`; return `${Math.floor(seconds / 86400)} days old`; }
...
export default function SupabaseObservabilityPage() {
  ...
  const tableColumns = useMemo<DataTableColumn<TableRow>[]>(() => [
    { id: "name", header: "Data area", accessor: (row) => row.name, sortable: true, cell: (row) => <><div className="flex flex-wrap items-center gap-2 font-mono text-[12px] text-[#16252B]">{row.name}{row.is_legacy && <Status tone="warning">OLDER DATA</Status>}</div><div className="mt-1 text-[10px] text-[#49615F]">{groupLabel(row.group_name)}</div></> },
    { id: "records", header: "Records", accessor: (row) => row.row_count, sortable: true, cell: (row) => <span className="font-mono">{number(row.row_count)}</span> },
    { id: "protection", header: "Access protection", accessor: (row) => row.rls_enabled ? (row.policy_count ? "Protected" : "Needs review") : "Not protected", cell: (row) => row.rls_enabled ? <Status tone={row.policy_count ? "healthy" : "critical"}>{row.policy_count ? "Protected" : "Needs review"}</Status> : <Status tone="warning">Not protected</Status> },
    { id: "rules", header: "Access rules", accessor: (row) => row.policy_count, sortable: true, cell: (row) => <span className="font-mono">{number(row.policy_count)}</span> },
    { id: "size", header: "Storage used", accessor: (row) => row.approximate_size_bytes, sortable: true, cell: (row) => bytes(row.approximate_size_bytes) },
    { id: "checked", header: "Last checked", accessor: (row) => row.last_analyzed_at || "", sortable: true, cell: (row) => when(row.last_analyzed_at) },
  ], []);
    ...
    <div className="mx-auto max-w-[1500px] space-y-8">
      ...
      <Section title="What data PropAI stores" icon={Database} refreshed={data.generated_at} onRefresh={() => load(true)}>
        <Card className="overflow-hidden border-[rgba(22,37,43,.14)] bg-[#F6FBF9] shadow-[0_8px_22px_rgba(22,37,43,.05)]">
          ...
          <div className="hidden max-h-[620px] overflow-auto p-3 sm:block"><DataTable columns={tableColumns} data={tables} getRowId={(row) => row.name} pageSize={12} tone="light" footerLabel={`${tables.length} data areas shown`} toolbar={<span className="text-xs text-[#49615F]">Click a column heading to sort · use Columns to simplify the view</span>} /></div>
      ...
      <div className="grid gap-8 xl:grid-cols-2">
        ...
        <Section title="Queue and worker health" icon={Activity} refreshed={data.generated_at} onRefresh={() => load(true)}>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><Metric label="Extraction messages waiting" value={number(queues.extraction_pending_24h)} note={`Actionable messages inside the ${number(queues.extraction_window_hours || 24)}-hour window · oldest ${age(queues.extraction_oldest_pending_age_seconds)}`} tone={Number(queues.extraction_pending_24h) ? "warning" : "normal"} /><Metric label="Review jobs waiting" value={number(queues.queued)} note="Source-grounded repair work, separate from live ingestion" tone={Number(queues.queued) ? "warning" : "normal"} /><Metric label="Jobs that could not finish" value={number(Number(queues.no_source || 0) + Number(queues.failed || 0))} note="Jobs missing information or ending in error" tone={Number(queues.no_source || 0) + Number(queues.failed || 0) ? "critical" : "normal"} /><Metric label="Jobs sent for later review" value={number(attempts.dead_lettered)} note="Processing attempts that need attention" tone={Number(attempts.dead_lettered) ? "warning" : "normal"} /><Metric label="Account-boundary checks" value={number(queues.tenant_boundary_pending)} note="Records waiting for workspace review" tone={Number(queues.tenant_boundary_pending) ? "warning" : "normal"} /></div>
