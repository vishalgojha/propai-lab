...
export default function PipelineHealthPage() {
  ...
  return <main className="propai-admin-page min-w-0 w-full overflow-x-hidden"><div className="propai-admin-heading mx-auto w-full max-w-7xl min-w-0 px-4 pt-4 lg:px-7"><div className="propai-kicker">System observability</div><h1 className="mb-1 text-2xl font-semibold text-white">Pipeline Health</h1><p className="mb-3 text-sm text-zinc-500">Internal diagnostics for PropAI’s data pipeline.</p><GroupedTabs tabs={tabs} active={active} /></div>{active === "enrichment" ? <BuildingEnrichmentPage /> : active === "repair" ? <ExtractionRepairPage /> : active === "dedupe" ? <DedupeGatePage /> : active === "embeddings" ? <SemanticEmbeddingsPage /> : <AdminProvidersPage />}</main>;
}
