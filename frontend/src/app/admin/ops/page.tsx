...
export default function OpsAdminPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!agentReady) {
      setError("PropAI Ops is currently unavailable. Your request is still in the box; retry after the agent is online.");
      return;
    }
    setError(null);
  ...
  function addAttachments(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (unsupported) {
      setError("PropAI Ops attachments currently support images only.");
      return;
    }
      ...
      <header className="flex shrink-0 items-center gap-3 border-b border-[var(--border)] pb-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h1 className="truncate text-base font-semibold tracking-[-0.02em] text-[var(--text-primary)]">PropAI Ops</h1>
