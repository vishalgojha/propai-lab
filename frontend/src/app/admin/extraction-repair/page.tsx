...
export default function ExtractionRepairPage() {
  ...
  return <div className="mx-auto max-w-7xl p-6 lg:p-8">
    <div className="mb-6 flex items-start justify-between gap-4">
      <div className="flex items-start gap-4"><Link href="/admin/pipeline-health?tab=providers" className="mt-1 text-zinc-400 hover:text-white"><ArrowLeft className="h-5 w-5" /></Link><div><h2 className="flex items-center gap-3 text-3xl font-semibold tracking-[-0.035em] text-white"><Scissors className="h-7 w-7 text-cyan-300" />Extraction boundary repair</h2><p className="mt-1 max-w-3xl text-sm text-zinc-500">Find historical parent broadcasts that skipped deterministic slicing, create immutable child evidence rows, and stop the stale parent from appearing in the market.</p></div></div>
    ...
    {preview && <section className="mb-6 overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/50"><div className="border-b border-white/10 p-5"><div className="flex items-center gap-2 font-semibold text-white"><CheckCircle2 className="h-4 w-4 text-emerald-300" />{preview.dry_run ? "Boundary preview" : `${preview.repaired_count} parent messages repaired`}</div><p className="mt-1 text-xs text-zinc-500">{preview.dry_run ? "Only rows marked repairable will be changed when you run the repair." : "The extraction worker will process the child rows on its next poll."}</p></div><div className="divide-y divide-white/5">{preview.preview.map((item) => <div key={item.raw_id} className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><span className="text-zinc-200">Raw #{item.raw_id}</span><span className="text-zinc-500">{item.pattern_id || "no deterministic pattern"} · {item.chunk_count} slice{item.chunk_count === 1 ? "" : "s"}</span><span className={item.status === "repairable" ? "text-emerald-300" : "text-zinc-500"}>{item.status}</span></div>)}</div></section>}
    <section className="overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/50"><div className="flex items-center gap-2 border-b border-white/10 p-5 font-semibold text-white"><ShieldAlert className="h-4 w-4 text-amber-300" />Recent repair jobs</div>{!state?.recent_jobs?.length ? <div className="p-8 text-center text-sm text-zinc-500">No historical repair jobs have been queued.</div> : <div className="divide-y divide-white/5">{state.recent_jobs.map((job) => <div key={job.id} className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><div><span className="text-white">Job #{job.id}</span><span className="ml-3 text-zinc-500">parent raw #{job.parent_raw_id}</span></div><span className="text-cyan-200">{job.status}</span><span className="text-zinc-500">{job.pattern_id || "—"} · {(job.child_raw_ids || []).length} children</span></div>)}</div>}</section>
  </div>;
}
