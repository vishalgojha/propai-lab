-- Keep provider execution completion separate from canonical evidence confirmation.
...
with job_counts as (
  ...
  from (
    select j.id, j.status, j.provider, j.priority, j.attempts, j.max_attempts,
           ...
           case when b.address is not null or b.google_place_id is not null
                ...
                when latest.action = 'needs_review' then 'needs_review'
  ...
  from (
    select h.id, h.job_id, h.provider, h.action, h.fields_updated, h.confidence,
           h.details, h.created_at, b.building_id as building_code,
  ...
  select count(*) filter (where action = 'web_search_attempt')::integer as attempts,
  ...
  from (
    ...
    from public.building_enrichment_history h
    left join public.buildings b on b.id = h.building_id
    where h.provider = 'crawl4ai' order by h.created_at desc limit 40
  ) x
), latest_success as (
  select max(completed_at) filter (where status = 'completed') as completed_at
  from public.building_enrichment_jobs
), latest_failure as (
  select jsonb_build_object('id', j.id, 'status', j.status, 'provider', j.provider,
           'last_error', j.last_error, 'attempts', j.attempts,
           'updated_at', coalesce(j.completed_at, j.started_at, j.created_at),
           'building_code', b.building_id, 'canonical_name', b.canonical_name) as value
  from public.building_enrichment_jobs j
  left join public.buildings b on b.id = j.building_id
  where j.status = 'failed'
  order by coalesce(j.completed_at, j.started_at, j.created_at) desc nulls last limit 1
)
select jsonb_build_object(
  'worker', coalesce((select to_jsonb(h) from public.worker_heartbeats h
    where h.worker_name = 'building-enrichment-worker'),
    jsonb_build_object('worker_name', 'building-enrichment-worker',
      'service_name', 'building-enrichment-worker', 'status', 'unknown', 'heartbeat_at', null)),
