-- Keep the dashboard health snapshot bounded. Retrieval correctness is measured
-- by semantic_retrieval_eval_cases; it must not perform 20 nearest-neighbour
-- scans on every status refresh.
...
as $$
  with job_rows as materialized (
    select source_table, source_id, entity_type, status, attempts,
  ...
  structural_quality as (
    select
      ...
      (select count(*)::bigint
       from public.semantic_embeddings v
       where not exists (
         select 1 from job_rows j
         where j.source_table = v.source_table and j.source_id = v.source_id
       )) as orphan_vectors,
      (select count(*)::bigint
       from public.semantic_embeddings
  ...
  alias_quality as (
    select jsonb_build_object(
      'building_aliases', jsonb_build_object('tested', 0, 'hit_at_5', 0, 'hit_rate_at_5', 0),
      'broker_aliases', jsonb_build_object('tested', 0, 'hit_at_5', 0, 'hit_rate_at_5', 0)
    ) as value
  ),
  latest_failure as (
    select jsonb_build_object(
  ...
  select jsonb_build_object(
    'jobs', jsonb_build_object(
      'total', jt.total, 'pending', jt.pending, 'running', jt.running,
