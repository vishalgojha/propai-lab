-- The intent-projection wrapper previously asked the legacy feed for 500 rows
-- on every request, even when the inbox requested 50 or a single broker had
-- only a handful of posts. Keep enough rows for the requested page, but cap
-- the pre-projection work so the broker refresh cannot run for minutes.
    ...
    select value as item
      from jsonb_array_elements(
        public.get_market_observations_feed_legacy(
          greatest(least(coalesce(p_limit, 50) + greatest(coalesce(p_offset, 0), 0), 100), 1),
          0, p_broker_key, '', p_tenant_id
        )
      )
), projected as (
    select item || case
      when exists (
        select 1
          from (
            select raw_message_id, listing_index from public.residential_sale_requirements
            union all select raw_message_id, listing_index from public.residential_rent_requirements
            union all select raw_message_id, listing_index from public.commercial_sale_requirements
            union all select raw_message_id, listing_index from public.commercial_rent_requirements
          ) requirements
      ...
      ) then jsonb_build_object('observation_type', 'REQUIREMENT', 'intent', 'BUY')
