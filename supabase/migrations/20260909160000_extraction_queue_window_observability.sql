-- Expose the live extraction queue without counting expired historical rows.
-- The worker has a finite 24-hour parsing window; this signal is intentionally
-- aligned with that window so Super Admin reflects actionable work only.
...
declare
  ...
  v_extraction_pending bigint := 0;
  v_extraction_oldest timestamptz;
begin
  perform set_config('statement_timeout', '60000', true);

  select count(*)::bigint, min(r.created_at)
    into v_extraction_pending, v_extraction_oldest
    from public.raw_messages r
   where r.processed = false
     and r.extraction_suppressed = false
     and r.is_group = true
  ...
  v_queue := v_queue || jsonb_build_object(
    'extraction_pending_24h', v_extraction_pending,
    'extraction_oldest_pending_at', v_extraction_oldest,
    'extraction_oldest_pending_age_seconds',
      case when v_extraction_oldest is null then null
           else greatest(0, extract(epoch from (clock_timestamp() - v_extraction_oldest))) end,
    'extraction_window_hours', 24
  );
