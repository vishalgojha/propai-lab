-- Workspace semantic retrieval must not treat a missing tenant as global.
-- A NULL tenant is only global for locality reference data and explicitly
-- shared listing/requirement inventory. A NULL p_tenant_id is reserved for
-- the service-role Super Admin probe, which intentionally inspects all tenants.
  ...
  with candidates as materialized (
    select e.entity_type, e.source_table, e.source_id, e.tenant_id,
     ...
     where (p_entity_types is null or e.entity_type = any(p_entity_types))
       and (
         -- The no-tenant form is used only by the protected Super Admin probe.
         (p_tenant_id is null and e.tenant_id is not null)
         or e.tenant_id = p_tenant_id
         or (
           e.entity_type in ('listing', 'requirement')
           and e.metadata->>'visibility' = 'shared_market'
         )
         or (e.entity_type = 'locality' and e.tenant_id is null)
       )
