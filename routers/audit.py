...
async def audit_dashboard(user: dict = Depends(require_user)):
    ...
    total_parsed_today = storage.db.execute(
        "SELECT COUNT(*) FROM parsed_output_unified WHERE created_at >= ?", (today_start,)
    ).fetchone()[0]
...
def audit_groups_v2(
    ...
    has_parsed = _table_exists("parsed_output_unified")
...
async def audit_group_detail(
    ...
    broker_count = storage.db.execute("""
        SELECT COUNT(DISTINCT p.broker_name) FROM parsed_output_unified p
        JOIN raw_messages r ON r.id = p.raw_message_id
        WHERE r.tenant_id = ? AND (r.group_name = ? OR r.group_name = ?)
    ...
    markets = storage.db.execute("""
        SELECT DISTINCT p.micro_market FROM parsed_output_unified p
        JOIN raw_messages r ON r.id = p.raw_message_id
        WHERE r.tenant_id = ? AND (r.group_name = ? OR r.group_name = ?)
...
async def audit_intelligence_v2(user: dict = Depends(require_user)):
    ...
    total_parsed = _audit_scalar("SELECT COUNT(DISTINCT raw_message_id) FROM parsed_output_unified WHERE tenant_id = ?", (tenant_id,), 0) if _table_exists("parsed_output_unified") else 0
    ...
    parsed_today = _audit_scalar("SELECT COUNT(DISTINCT raw_message_id) FROM parsed_output_unified WHERE tenant_id = ? AND created_at >= ?", (tenant_id, today_start), 0) if _table_exists("parsed_output_unified") else 0
    ...
    total_listings = _audit_scalar("SELECT COUNT(*) FROM listings_unified WHERE tenant_id = ?", (tenant_id,), 0) if _table_exists("listings_unified") else 0
    sell_count = _audit_scalar("SELECT COUNT(*) FROM listings_unified WHERE tenant_id = ? AND upper(COALESCE(intent, '')) IN ('SELL','SELLER','SALE','COMMERCIAL_SALE','PRE-LAUNCH')", (tenant_id,), 0) if _table_exists("listings_unified") else 0
    rent_count = _audit_scalar("SELECT COUNT(*) FROM listings_unified WHERE tenant_id = ? AND upper(COALESCE(intent, '')) IN ('RENT','RENTAL','LEASE','COMMERCIAL_RENTAL')", (tenant_id,), 0) if _table_exists("listings_unified") else 0
    commercial_count = _audit_scalar("SELECT COUNT(*) FROM listings_unified WHERE tenant_id = ? AND upper(COALESCE(intent, '')) LIKE '%COMMERCIAL%'", (tenant_id,), 0) if _table_exists("listings_unified") else 0
    total_requirements = _audit_scalar("""
        SELECT COUNT(DISTINCT raw_message_id) FROM parsed_output_unified
        WHERE tenant_id = ? AND upper(COALESCE(intent, '')) IN ('BUY','BUYER','REQUIREMENT','RENTAL_SEEKER','TENANT')
    """, (tenant_id,), 0) if _table_exists("parsed_output_unified") else 0

    markets_observed = _audit_scalar("SELECT COUNT(DISTINCT micro_market) FROM parsed_output_unified WHERE tenant_id = ? AND COALESCE(micro_market, '') != ''", (tenant_id,), 0) if _table_exists("parsed_output_unified") else 0
    ...
    developers_observed = _audit_scalar("SELECT COUNT(DISTINCT developer) FROM parsed_output_unified WHERE tenant_id = ? AND COALESCE(developer, '') != ''", (tenant_id,), 0) if _table_exists("parsed_output_unified") else 0
    localities_observed = _audit_scalar("SELECT COUNT(DISTINCT area) FROM parsed_output_unified WHERE tenant_id = ? AND COALESCE(area, '') != ''", (tenant_id,), 0) if _table_exists("parsed_output_unified") else 0
    landmarks_observed = _audit_scalar("SELECT COUNT(DISTINCT landmark_name) FROM parsed_output_unified WHERE tenant_id = ? AND COALESCE(landmark_name, '') != ''", (tenant_id,), 0) if _table_exists("parsed_output_unified") else 0
    ...
    group_rows = []
    if _table_exists("raw_messages") and _table_exists("parsed_output_unified"):
        group_rows = _audit_rows("""
            ...
            LEFT JOIN parsed_output_unified po ON po.raw_message_id = rm.id
    ...
    broker_reach = _audit_rows("""
        ...
        LIMIT 10
    """, (tenant_id,)) if _table_exists("parsed_output_unified") and _table_exists("raw_messages") else []
...
def audit_insights(
    ...
    try:
        ...
        flow_rows = _audit_rows(
            ...
            "FROM raw_messages rm JOIN parsed_output_unified po ON po.raw_message_id = rm.id "
            "WHERE rm.tenant_id = $1 AND rm.created_at >= $2 "
            "GROUP BY (rm.created_at)::date ORDER BY day",
            (tenant_id, week_ago),
        ) if _table_exists("raw_messages") and _table_exists("parsed_output_unified") else []
        ...
        market_rows = _audit_rows(
            ...
            "FROM parsed_output_unified po JOIN raw_messages rm ON po.raw_message_id = rm.id "
