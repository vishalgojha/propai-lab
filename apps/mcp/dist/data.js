...
import { formatBudgetRange, formatCurrencyCr, formatPerSqft, listingLabel, toNumber } from "./format.js";
export const PUBLIC_LISTING_COLUMNS = "source_message_id, source_group_name, listing_type, area, sub_area, location, price, price_type, size_sqft, furnishing, bhk, property_type, title, description, raw_message, cleaned_message, primary_contact_name, primary_contact_number, primary_contact_wa, message_timestamp";
const PARSED_MARKET_COLUMNS = "id, raw_message_id, listing_index, message_type, intent, bhk, price, price_unit, area_sqft, furnishing, location_raw, area, micro_market, building_name, broker_name, broker_phone, profile_name, raw_payload, summary_title, created_at, raw_messages(group_name, sender, sender_phone, message, timestamp)";
...
function normalizeParsedPrice(value, unit) {
    ...
    if (["lac", "lakh", "lakhs", "l"].includes(normalizedUnit))
        return price * 100_000;
    if (["k", "thousand"].includes(normalizedUnit))
        return price * 1_000;
    return price;
}
...
function inferListingTypeFromParsed(row, rawText) {
    const text = [
        row.intent,
        row.message_type,
        row.price_unit,
    ...
    if (/(requirement|wanted|looking|need|tenant|buyer|rent req|lease req)/.test(text))
        return "requirement";
    if (/(rent|lease|monthly|tenant)/.test(text))
        return "listing_rent";
    if (/(sale|sell|outright|resale|distress|auction|cr\b|crore)/.test(text))
        return "listing_sale";
    return "listing";
}
function parsedSourceId(row) {
...
function sourceIdParts(listingId) {
...
function mapParsedRowToPublicListing(row) {
    ...
    const fullText = firstString(payload.full_text, payload.text, payload.message, row.summary_title, raw.message);
...
function inferPublicDealType(row) {
    const lower = [
        row.listing_type,
        row.property_type,
        row.price_type,
    ...
    if (lower.includes("requirement"))
        return "requirement";
    if (lower.includes("rent") || lower.includes("lease") || lower.includes("monthly"))
        return "rent";
    if (lower.includes("sale") || lower.includes("outright"))
        return "sale";
    return "unknown";
}
function normalizePublicListingRow(row) {
    ...
    if (price != null && Number.isFinite(price) && price > 0) {
        if (dealType === "rent" && price > 5_000_000)
            return null;
        if (dealType === "rent" && price < 5_000)
            return null;
        if (dealType === "sale" && price > 500_000_000)
            return null;
        if (dealType === "unknown") {
...
function dedupePublicListings(rows) {
    const seen = new Set();
    const deduped = [];
...
export function normalizePublicListings(rows) {
    return dedupePublicListings(rows
        .map((row) => ({
        ...row,
...
function applyLocality(query, locality, city) {
    const terms = [locality, city].map((value) => value?.trim()).filter(Boolean);
    for (const term of terms) {
        query = query.or(`area.ilike.%${term}%,sub_area.ilike.%${term}%,location.ilike.%${term}%`);
    }
    return query;
}
function applyBudget(query, maxBudgetCr) {
    if (maxBudgetCr == null)
        return query;
...
function applyListingType(query, requested, fallback) {
...
function filterPublicListingRows(rows, input) {
    ...
    return rows.filter((row) => {
        ...
        if (terms.length && !terms.every((term) => haystack.includes(term)))
            return false;
        const dealType = inferPublicDealType(row);
        if (input.listingKind === "requirement" && dealType !== "requirement")
            return false;
        if (input.listingKind === "listing" && dealType === "requirement")
            return false;
        if (propertyType && propertyType !== "all" && dealType !== propertyType)
            return false;
        if (input.bhk != null && row.bhk !== input.bhk)
            return false;
        if (maxBudget != null && row.price != null && row.price > maxBudget * 10_000_000)
            return false;
        if (input.budget_min_cr != null && row.price != null && row.price < input.budget_min_cr * 10_000_000)
            return false;
        return true;
    });
}
async function fetchParsedMarketRows(limit, since) {
    let query = supabase
        .from("parsed_output")
    ...
    return (data || []).map(mapParsedRowToPublicListing);
}
export async function logToolCall(brokerId, toolName, input) {
...
export async function getFreshStream(input) {
...
function mcpFingerprint(prefix, brokerId, text) {
    return crypto
        .createHash("sha1")
        .update([prefix, brokerId, normalizeListingText(text)].join("|"))
        .digest("hex");
}
function splitDueDateTime(value) {
...
function parseBudgetToCr(value) {
...
function inferUrgency(text) {
    if (/\b(urgent|immediate|today|asap|closing|hot)\b/i.test(text))
        return "high";
    if (/\b(this week|soon|priority|follow up)\b/i.test(text))
        return "medium";
    return "low";
}
function inferPriorityBucket(urgency) {
    if (urgency === "high")
        return "P1";
    if (urgency === "medium")
        return "P2";
    ...
    if (input.recordType === "buyer_requirement") {
        const { data: requirementRow, error: requirementError } = await supabase
            ...
            bhk: Array.isArray(input.payload?.bhk_preference)
                ? (input.payload?.bhk_preference).join(", ")
                : null,
...
export async function getBrokerActivity(input) {
    ...
    for (const row of [...listings, ...requirements]) {
        const locality = String(row.micro_market || row.location_label || "").trim();
        if (!locality)
            continue;
...
export async function getHotLeadTriage(input) {
    ...
    const followUps = followUpResult.data || [];
    const now = Date.now();
    const followUpByClientId = new Map();
    const followUpByPhone = new Map();
    for (const item of followUps) {
        if (item.client_id)
            followUpByClientId.set(Number(item.client_id), item);
        if (item.broker_phone)
            followUpByPhone.set(item.broker_phone, item);
    }
    const scored = requirements.map((lead) => {
        ...
        if (lead.micro_market)
            score += 6;
        if (lead.price_max != null)
            score += 4;
        const why = [
            followUp?.due_date
...
export async function getRecentSavedListings(input) {
    return getWorkspaceListings(input);
}
export async function getRecentRequirements(input) {
    const { data, error } = await supabase
        .from("client_requirements")
...
function isValidBuildingName(raw) {
    if (!raw)
        return false;
...
function computePriceBenchmark(rows) {
    const withSqft = rows.filter((r) => r.price_numeric != null && r.area_sqft != null && r.area_sqft > 0);
...
export async function getBuildingIntel(input) {
    ...
    for (const row of listings) {
        ...
        if (!cfg || /^n\/?a$/i.test(cfg) || cfg === "-")
            continue;
        if (!configMap.has(cfg))
            configMap.set(cfg, new Set());
...
export async function getListingById(listingId) {
    ...
    if (parts.rawId != null) {
        ...
        if (parts.index != null)
            query = query.eq("listing_index", parts.index);
    }
    else {
    ...
    if (!data)
        return null;
    const normalized = normalizePublicListings([mapParsedRowToPublicListing(data)]);
