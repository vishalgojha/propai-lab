import { z } from "zod";
import { getStoredThreadMessages, summarizeThread, logToolCall } from "../data.js";
...
export function registerInboxTools(server, context) {
    server.registerTool("conversation_search", {
        ...
        if (!messages.length)
            return textResponse("No stored conversations found.", { conversations: [] });
    ...
    server.registerTool("conversation_timeline", {
        ...
        if (!messages.length)
            return textResponse(`No messages found for "${input.remote_jid}".`, { messages: [] });
    ...
    server.registerTool("conversation_summarize", {
        ...
        const keyPoints = result.key_points?.slice(-3).map((kp) => `• ${kp.sender}: ${kp.text.slice(0, 120)}`).join("\n") || "";
        return textResponse(`${result.message_count} messages · ${result.inbound_count} in / ${result.outbound_count} out\nParticipants: ${result.participants?.join(", ") || "N/A"}\n${keyPoints ? `\nKey points:\n${keyPoints}` : ""}`, result);
    });
    server.registerTool("conversation_reply", {
        ...
        return textResponse(`Draft for ${input.remote_jid}: "${input.message}"\n\nSending requires Evolution API integration — your draft is ready for manual sending.`, { remote_jid: input.remote_jid, draft: input.message, sent: false });
    });
}
