...
export function registerMarketTools(server, context) {
    ...
    server.registerTool("market_summary", {
        ...
        return textResponse(`${input.location || "Mumbai"}: ${result.listing_count} listings, avg ₹${result.avg_price_cr || "N/A"} Cr, avg ₹${result.avg_price_per_sqft || "N/A"}/sqft`, result);
    });
    server.registerTool("market_trends", {
        ...
        const top = result.top_localities?.map((l) => `${l.locality} (${l.count})`).join(", ") || "";
        return textResponse(`${input.location || "Mumbai"} over ${input.days}d: ${result.listing_count} listings, avg ₹${result.avg_price_cr || "N/A"} Cr${top ? `\nTop localities: ${top}` : ""}`, result);
    });
}
