import { z } from "zod";
import { getBuildingIntel, logToolCall } from "../data.js";
...
const fmtPpsf = (b) => b ? `₹${b.avg_price_per_sqft || "N/A"}/sqft` : "N/A";
const fmtPpsfRange = (b) => b ? `₹${b.min_price_per_sqft || "N/A"}–${b.max_price_per_sqft || "N/A"}/sqft` : "N/A";
export function registerBuildingTools(server, context) {
    server.registerTool("building_search", {
        ...
        if (!result.building_name)
            return textResponse(`"${input.query}" not found.`, { building: null });
        const ppsf = result.price_benchmarks?.sale;
        return textResponse(`${result.building_name}\nLocalities: ${result.matched_localities?.join(", ") || "N/A"}\nSale: ${fmtPpsf(ppsf)}`, result);
    });
    server.registerTool("building_profile", {
        ...
        if (!result.building_name)
            return textResponse(`No data for "${input.building_name}".`, null);
        const sale = result.price_benchmarks?.sale;
        const rent = result.price_benchmarks?.rent;
        return textResponse(`${result.building_name} @ ${result.matched_localities?.join(", ") || "N/A"}\nSale: ${fmtPpsfRange(sale)} (${sale?.listing_count || 0} samples)\nRent: ${fmtPpsfRange(rent)} (${rent?.listing_count || 0} samples)`, result);
    });
    server.registerTool("building_inventory", {
        ...
        const result = await getBuildingIntel({ building_name: input.building_name, locality: input.location, days_back: input.days_back });
        if (!result.building_name)
            return textResponse(`No inventory for "${input.building_name}".`, null);
        const configs = result.configuration_map?.map((c) => `${c.configuration}: ${c.count} (${c.percentage_of_locality}%)`).join("\n") || "None";
    ...
    server.registerTool("building_requirements", {
        ...
        if (!result.building_name)
            return textResponse(`No data for "${input.building_name}".`, null);
        const supply = result.locality_supply?.map((l) => `${l.locality}: ${l.listings} listings, ${l.requirements} requirements (${l.ratio})`).join("\n") || "None";
    ...
    server.registerTool("building_marketPulse", {
        ...
        if (!intel.building_name)
            return textResponse(`No pulse data for "${input.building_name}".`, null);
        const sale = intel.price_benchmarks?.sale;
        const rent = intel.price_benchmarks?.rent;
        const supplyTotal = intel.locality_supply?.reduce((s, l) => s + (l.listings || 0), 0) || 0;
        const reqTotal = intel.locality_supply?.reduce((s, l) => s + (l.requirements || 0), 0) || 0;
        return textResponse(`${intel.building_name} @ ${intel.matched_localities?.join(", ") || "N/A"}\n${supplyTotal} listings / ${reqTotal} requirements (${intel.sample_days}d)\nSale: ${fmtPpsfRange(sale)}\nRent: ${fmtPpsfRange(rent)}`, intel);
    });
}
