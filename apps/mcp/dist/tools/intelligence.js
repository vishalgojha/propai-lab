import { z } from "zod";
import { getMarketSummary, getBuildingIntel, logToolCall } from "../data.js";
...
export function registerIntelligenceTools(server, context) {
    server.registerTool("intel_ask", {
        ...
        if (summary)
            parts.push(`${location}: ${summary.listing_count} listings, avg ₹${summary.avg_price_cr || "N/A"} Cr`);
    ...
    server.registerTool("intel_compare", {
        ...
        const fmt = (x) => input.type === "building"
