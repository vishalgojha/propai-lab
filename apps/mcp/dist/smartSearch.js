...
import { searchPublicListings, getMarketSummary, getBuildingIntel } from "./data.js";
import { formatCurrencyCr } from "./format.js";
...
function toCr(amount, unit) {
    const u = (unit || "cr").toLowerCase();
    if (["cr", "crore", "crores"].includes(u))
        return amount;
    if (["lakh", "lakhs", "lac", "lacs"].includes(u))
        return amount / 100;
    if (["k", "thousand"].includes(u))
        return amount / 10000;
    return amount;
}
...
function buildExplanation(intent, params, count) {
    const parts = [];
    if (intent === "listing_search")
        parts.push("Find matching listings");
    else if (intent === "requirement_search")
        parts.push("Find matching requirements");
    else if (intent === "broker_search")
        parts.push("Search brokers");
    else if (intent === "market_insights")
        parts.push("Market intelligence");
    else if (intent === "fresh_stream")
        parts.push("Recent listings");
    else if (intent === "building_intel")
        parts.push("Building intel");
    else
        parts.push("Search");
    const filters = [];
    if (params.propertyType)
        filters.push(params.propertyType === "sale" ? "for sale" : "for rent");
    if (params.locality)
        filters.push(`in ${params.locality}`);
    if (params.city)
        filters.push(params.city);
    if (params.bhk)
        filters.push(`${params.bhk} BHK`);
...
function buildFollowUps(intent, params) {
    const suggestions = [];
    if (intent === "listing_search") {
        if (params.locality) {
...
export async function executeSmartSearch(input) {
    ...
    switch (intent) {
        ...
        case "market_insights": {
            ...
            const insights = summary.top_localities.length
                ? summary.top_localities.slice(0, 5).map((t) => `${t.locality}: ${t.count} listings`).join("\n")
        ...
        case "broker_search": {
            ...
            const rows = (profiles || []).filter((p) => p.app_role === "broker" || p.app_role === "super_admin");
            return {
                intent,
                query,
                params: { ...extracted, locality, city, limit },
