...
import { buildBroadcastDraft, createRequirementRecord, describeSearch, estimatePrice, getBrokerActivity, getBuildingIntel, getFreshStream, getHotLeadTriage, getListingById, getMarketSummary, matchBuyerToInventory, getStaleLeadReactivation, buildPricingNegotiationBrief, logToolCall, qualifyLead, saveListingRecord, scheduleFollowUp, searchBrokers, searchPublicListings, summarizeThread, } from "./data.js";
...
import { executeSmartSearch } from "./smartSearch.js";
import { registerMarketTools } from "./tools/market.js";
import { registerListingTools } from "./tools/listing.js";
import { registerRequirementTools } from "./tools/requirement.js";
...
import { LISTING_CARDS_URI } from "./uiResources.js";
...
function listingCardPayload(row) {
...
export function createMcpServer(context = {}) {
    ...
    server.registerTool("smartSearch", {
        description: "Search Mumbai real estate inventory, requirements, brokers, and market intelligence using natural language. This is the PRIMARY tool — always use it first when the user asks about properties, listings, requirements, market rates, localities, or broker information. It internally understands intent and returns the most relevant results.",
        inputSchema: {
            query: z.string().describe("Natural language query. Examples: '3 BHK for sale in Bandra under 8 crore', 'rental requirements in Khar West above 1 lakh', 'Which locality has the strongest rental demand?', 'brokers dealing in Powai', 'market rate for Kalpataru Magnus'"),
        ...
        if (result.intent === "listing_search" || result.intent === "fresh_stream") {
            for (let i = 0; i < items.length; i++) {
                const r = items[i];
                lines.push(listingLine(r, i));
            }
        }
        else if (result.intent === "requirement_search") {
            for (let i = 0; i < items.length; i++) {
                const r = items[i];
                lines.push(`${i + 1}. ${r.title || "Requirement"} — ${r.sub_area || r.area || r.location || "?"} — ${formatCurrencyCr(r.price)}`);
            }
        }
    ...
    server.registerTool("getListing", {
        description: "Retrieve complete details for a specific listing by its source_message_id. Use this when you have a listing ID from a previous search result.",
        inputSchema: {
        ...
        const details = [
            ...
            `Price: ${r.price != null ? formatCurrencyCr(r.price) : "N/A"}`,
            `Area: ${r.size_sqft != null ? formatSqft(r.size_sqft) : "N/A"}`,
    ...
    server.registerTool("searchBrokers", {
        description: "Search brokers within the PropAI network by city, locality, or specialization.",
        inputSchema: {
        ...
        const lines = rows.map((r, i) => `${i + 1}. ${r.full_name || "Unknown"} — ${r.city || ""} — ${r.agency_name || ""} — ${r.phone || r.email || ""}`);
        return textResponse(`Found ${rows.length} broker(s):\n\n${lines.join("\n")}`, { results: rows });
    });
    server.registerTool("draft_growth_asset", {
        description: "Draft GTM or marketing copy for PropAI such as launch posts, broker pitches, partner outreach, or case-study style summaries.",
        inputSchema: {
        ...
        return textResponse(`${result.title}\n\n${result.body}\n\nCTA: ${result.CTA}\nAngle: ${result.angle}`, result);
    });
    server.registerTool("extract_thread_actions", {
        description: "Extract likely CRM actions from a stored WhatsApp thread: buyer requirements, listings, follow-ups, and unresolved questions.",
        inputSchema: {
    ...
    server.registerTool("stale_lead_reactivation", {
        description: "Find stale leads that are worth reactivating and draft a practical re-engagement opener for each one.",
        inputSchema: {
        ...
        return textResponse(`Found ${result.items.length} stale leads worth reactivating:\n\n${lines.join("\n")}`, result);
    });
    server.registerTool("pricing_negotiation_brief", {
        description: "Build a pricing and negotiation brief using current asking price and live PropAI market comparables.",
        inputSchema: {
    ...
    server.registerTool("buyer_to_inventory_match", {
        description: "Match a buyer brief to current inventory from the PropAI broker network, workspace CRM, or both, with explainable ranking.",
        inputSchema: {
        ...
        return textResponse(`Found ${result.items.length} ranked buyer-to-inventory matches:\n\n${lines.join("\n")}`, result);
    });
    server.registerTool("save_thread_requirement", {
        description: "Persist one extracted thread requirement candidate into the broker CRM.",
        inputSchema: {
        ...
        return textResponse(`Thread requirement saved${result.lead?.lead_id ? ` with lead id ${result.lead.lead_id}` : ""} for ${input.location_pref || "the requested location"}.`, result);
    });
    server.registerTool("save_thread_listing", {
        description: "Persist one extracted thread listing candidate into the broker CRM.",
        inputSchema: {
        ...
        return textResponse(`Thread listing saved${result.listing_id ? ` with id ${result.listing_id}` : ""} for ${result.listing.location || "the requested location"}.`, result);
    });
    server.registerTool("create_thread_follow_up", {
        description: "Create one follow-up task from an extracted thread action candidate.",
        inputSchema: {
        ...
        return textResponse(`Thread follow-up scheduled for ${input.lead_name} at ${result.due_at}.`, result);
    });
    server.registerTool("triage_hot_leads", {
        description: "Rank the broker's hottest leads by urgency, follow-up pressure, and recent activity so they know what to handle first.",
        inputSchema: {
        ...
        return textResponse(`Hot lead triage for the last ${result.days} days:\n\n${lines.join("\n")}`, result);
    });
    server.registerTool("broker_activity_legacy", {
        description: "Summarize the broker's recent PropAI activity: lead volume, active chats, follow-up queue, and top localities.",
        inputSchema: {
            days: z.number().default(7).describe("Look back window in days"),
        },
    }, async (input) => {
        ...
        const nextFollowUp = result.next_follow_up
            ...
            : "none scheduled";
        return textResponse(`Last ${result.days} days: ${result.leads_total} leads (${result.listings_total} listings, ${result.requirements_total} requirements), ${result.messages_total} messages across ${result.active_chats} chats, ${result.pending_follow_ups} pending follow-ups. Next follow-up: ${nextFollowUp}. Top localities: ${topLocalities}.`, result);
    });
    server.registerTool("create_requirement", {
        description: "Create and store a buyer or tenant requirement in the broker's workspace CRM.",
        inputSchema: {
        ...
        const result = await createRequirementRecord({ brokerId: id, ...input });
        return textResponse(`Requirement saved for ${input.location_pref || "the requested location"} with lead id ${result.lead.lead_id}.`, result);
    });
    server.registerTool("match_requirement_to_broker", {
        description: "Match a buyer or tenant requirement to brokers who have suitable listings in the PropAI broker network, workspace CRM, or both.",
        inputSchema: {
        ...
        return textResponse(`Found ${result.items.length} broker matches for this requirement:\n\n${lines.join("\n")}`, result);
    });
    server.registerTool("draft_broadcast", {
        description: "Draft a broadcast-ready WhatsApp listing message without sending it.",
        inputSchema: {
    ...
    server.registerTool("market_summary_legacy", {
        description: "Summarize listing market activity for a locality, city, deal type, or BHK from PropAI's public stream.",
        inputSchema: {
            locality: z.string().optional(),
            city: z.string().optional(),
        ...
        return textResponse(`Market summary for the last ${result.days} days: ${result.listing_count} comparable listings, average ${result.avg_price_cr != null ? formatCurrencyCr(result.avg_price_cr) : "price unavailable"}, median ${result.median_price_cr != null ? formatCurrencyCr(result.median_price_cr) : "price unavailable"}, average ${result.avg_price_per_sqft != null ? `₹${result.avg_price_per_sqft.toLocaleString("en-IN")}/sqft` : "ppsf unavailable"}. Top localities: ${topLocalities}.`, result);
    });
    server.registerTool("price_estimate", {
        description: "Estimate a property's price from live PropAI market comparables.",
        inputSchema: {
            locality: z.string().optional(),
            building_name: z.string().optional(),
            bhk: z.number().optional(),
    ...
    server.registerTool("qualify_lead", {
        description: "Save lead qualification fields like budget, locality, timeline, and possession, and score urgency.",
        inputSchema: {
        ...
        return textResponse(`Lead qualified with ${result.priority_bucket} priority and ${result.urgency} urgency.`, result);
    });
    server.registerTool("save_listing", {
        description: "Create and store a listing in the broker's workspace CRM.",
        inputSchema: {
        ...
        const result = await saveListingRecord({ brokerId: id, ...input });
        return textResponse(`Listing saved${result.listing_id ? ` with id ${result.listing_id}` : ""} for ${result.listing.location || "the requested location"}.`, result);
    });
    server.registerTool("set_follow_up", {
        description: "Schedule a callback, visit, or follow-up task for the broker.",
        inputSchema: {
            lead_id: z.string().optional(),
            lead_name: z.string(),
            lead_phone: z.string().optional(),
        ...
        const result = await scheduleFollowUp({ brokerId: id, ...input });
        return textResponse(`Follow-up scheduled for ${input.lead_name} at ${result.due_at}.`, result);
    });
    server.registerTool("summarise_thread", {
        description: "Summarize a WhatsApp thread from stored workspace message history.",
        inputSchema: {
    ...
    server.registerTool("search_listings", {
        description: "Search real estate listings from PropAI's live WhatsApp stream. Use when someone asks about available properties, flats, offices, or shops in a locality.",
        _meta: {
            ui: {
        ...
        if (!rows.length)
            return noResults("listings");
        const place = [input.locality, input.city].filter(Boolean).join(", ") || "your search";
        const lines = rows.map(listingLine);
        const listingCards = rows.map((row) => listingCardPayload(row));
        return {
            content: [{ type: "text", text: `Found ${rows.length} listings in ${place}:\n\n${lines.join("\n")}` }],
            structuredContent: {
                results: rows,
    ...
    server.registerTool("search_requirements", {
        description: "Find buyer/tenant requirements posted by brokers. Use when someone wants to know what buyers are looking for in a locality.",
        inputSchema: {
            locality: z.string().optional(),
            city: z.string().optional(),
            bhk: z.number().optional(),
    ...
    server.registerTool("match_listing_to_requirement", {
        description: "Find listings that match a specific requirement. Use when broker has a buyer and wants matching properties.",
        inputSchema: {
            locality: z.string().optional(),
            bhk: z.number().optional(),
            budget_min_cr: z.number().optional(),
        ...
        if (!rows.length)
            return noResults("matching listings");
        const summary = describeSearch(input);
        const lines = rows.map(listingLine);
    ...
    server.registerTool("semantic_search", {
        description: "Semantically search real estate listings using natural language. Use when someone describes what they want in plain English, e.g. 'a quiet 2BHK near the sea in Bandra with good ventilation under 3Cr'. Finds listings by meaning, not just keyword match.",
        inputSchema: {
            ...
            for (const table of []) {
                ...
                while (true) {
                    ...
                    for (const row of rows) {
                        ...
                        if (!vec)
                            continue;
                        const parsedVec = typeof vec === "string" ? JSON.parse(vec) : vec;
                        if (!Array.isArray(parsedVec) || parsedVec.length !== 768)
                            continue;
                        if (input.locality && !String(row.locality || "").toLowerCase().includes(input.locality.toLowerCase()))
                            continue;
                        if (input.bhk && String(row.bhk || "") !== String(input.bhk))
                            continue;
                        if (input.type && String(row.type || "").toLowerCase() !== input.type.toLowerCase())
                            continue;
        ...
        const lines = results.map((r) => `${r.bhk || "?"}BHK ${r.locality || "?"} — ${r.price_label || "?"} (${r.type || "?"}${r.furnishing ? `, ${r.furnishing}` : ""}) — ${Math.round(r.similarity * 100)}% match`);
        return textResponse(`Found ${results.length} semantically matching listings for "${input.query}":\n\n${lines.join("\n")}`, {
            results,
        });
    });
    server.registerTool("get_fresh_stream", {
        description: "Get the freshest listings and requirements from the last N hours. Use when broker wants to see what's new today.",
        inputSchema: {
        ...
        if (!rows.length)
            return noResults(`items from the last ${input.hours ?? 6} hours`);
        const lines = rows.map(listingLine);
    ...
    server.registerTool("building_intel", {
        description: "Get market intelligence for a building — price per sqft benchmarks, locality supply snapshot, and configuration demand map. Use when a broker asks about rates in a specific building or wants to understand the market for a locality.",
        inputSchema: {
    ...
    server.registerTool("search", {
        description: "Search Mumbai real estate listings, requirements, brokers, buildings, and market intelligence using natural language. Returns results with routable IDs for use with fetch().",
        inputSchema: {
            query: z.string().describe("Natural language query. Examples: '3 BHK for sale in Bandra under 8 crore', 'rental requirements in Khar West above 1 lakh', 'brokers dealing in Powai', 'market rate for Kalpataru Magnus'"),
        },
    }, async (input) => {
        const id = brokerId(context);
        ...
        const searchResults = items.map((r, index) => {
            const row = r;
            let resultId;
            let title;
            ...
            if (isRequirement) {
                ...
                title = `${row.title || "Requirement"} - ${row.sub_area || row.area || row.location || "Unknown"} - ${row.price != null ? formatCurrencyCr(row.price) : "Price on request"}`;
            }
            else if (isListing) {
                resultId = `listing:${String(row.source_message_id || `result-${index}`)}`;
                const priceStr = row.price != null ? formatCurrencyCr(row.price) : "Price on request";
    ...
    server.registerTool("fetch", {
        description: "Fetch full details for a result ID returned by search(). The ID format is type:value (e.g., listing:msg-123, requirement:msg-456, broker:broker-uuid, building:Building Name). Returns full human-readable content and optional metadata.",
        inputSchema: {
            ...
            switch (type) {
                case "listing": {
                    ...
                    if (!result) {
                        return {
                            ...
                            content: [{ type: "text", text: JSON.stringify({ error: `Listing "${value}" not found.` }) }],
                        };
                    }
                    const r = result;
                    const priceStr = r.price != null ? formatCurrencyCr(r.price) : "Price on request";
                    const areaStr = r.size_sqft != null ? formatSqft(r.size_sqft) : "N/A";
                    ...
                    return {
                        ...
                        content: [{ type: "text", text: JSON.stringify({ id: input.id, title: lines[0], text: lines.join("\n"), metadata: r }) }],
                    };
                }
                case "requirement": {
                    const result = await getListingById(value);
                    if (!result) {
                        return {
                            ...
                            content: [{ type: "text", text: JSON.stringify({ error: `Requirement "${value}" not found.` }) }],
                        };
                    }
                    const r = result;
                    const priceStr = r.price != null ? formatCurrencyCr(r.price) : "Budget not specified";
                    const lines = [
                ...
                case "broker": {
                    ...
                    if (!data) {
                        return {
                            ...
                            content: [{ type: "text", text: JSON.stringify({ error: `Broker "${value}" not found.` }) }],
                        };
                    }
                    const broker = data;
                ...
                default:
                    return {
                        ...
                        content: [{ type: "text", text: JSON.stringify({ error: `Unknown ID type: ${type}. Supported types: listing, requirement, broker, building, market` }) }],
                    };
            }
        }
        catch (error) {
...
export { searchPublicListings, getFreshStream, getBrokerActivity, getHotLeadTriage, getStaleLeadReactivation, matchBuyerToInventory, qualifyLead, saveListingRecord, createRequirementRecord, scheduleFollowUp, summarizeThread, extractThreadActionsWithLlm, buildPricingNegotiationBrief, estimatePrice, getMarketSummary, getBuildingIntel, buildBroadcastDraft, describeSearch, draftGrowthAssetWithLlm, summarizeBrokerThreadWithLlm, generateEmbedding, formatCurrencyCr, formatPerSqft, formatSqft, listingLine, getListingById, searchBrokers, executeSmartSearch, };
