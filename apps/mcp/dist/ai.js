...
function fallbackSummary(lines) {
...
function parseSummary(raw, fallback) {
    try {
...
function fallbackThreadActions(lines) {
...
function parseThreadActions(raw, fallback) {
    try {
        const parsed = JSON.parse(raw);
...
export async function summarizeBrokerThreadWithLlm(input) {
    ...
    if (openRouterKey) {
        try {
            const raw = await callOpenAICompatible(OPENROUTER_BASE_URL, openRouterKey, OPENROUTER_MODEL, messages, {
    ...
    if (groqKey) {
        try {
            const raw = await callOpenAICompatible(GROQ_BASE_URL, groqKey, GROQ_MODEL, messages);
            return parseSummary(raw, fallback);
        }
        catch {
...
function parseGrowthDraft(raw, fallback) {
    try {
        const parsed = JSON.parse(raw);
