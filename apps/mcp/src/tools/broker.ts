...
export function registerBrokerTools(server: McpServer, context: ToolContext) {
  server.registerTool("broker_search", {
    ...
    const summary = brokers.map((b: any) => `${b.full_name || "Unknown"}${b.phone ? ` · ${b.phone}` : ""}`).join("\n");
  ...
  server.registerTool("broker_activity", {
    ...
    return textResponse(
      `Activity over ${input.days}d: ${result.leads_total} total leads, ${result.listings_total} listings, ${result.requirements_total} requirements, ${result.messages_total} messages, ${result.pending_follow_ups} pending follow-ups`,
      result,
    );
  });
  ...
  server.registerTool("broker_inventory", {
    ...
    return textResponse(
      `Your inventory (90d): ${activity.listings_total} listings, ${activity.requirements_total} requirements, ${activity.pending_follow_ups} pending follow-ups`,
      activity,
    );
  });
}
