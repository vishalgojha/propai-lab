      ...
      <main className="max-w-[1600px] mx-auto px-4 lg:px-6 py-10 lg:py-14">
        ...
        <header className="mb-10 space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="h-14 w-28 rounded-xl skeleton" />
            ))}
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-44 rounded-2xl border border-white/10 bg-zinc-950/80 skeleton" />
          ))}
        </div>

        <div className="mt-12 h-64 rounded-2xl border border-white/10 bg-zinc-950/80 skeleton" />
      </main>
      <SiteFooter />
    </div>
  );
