...
- Verification: `python3 -m py_compile extraction_quality.py extraction.py`; focused confidence and semantic tests passed (`4 passed`); scoped diff checks passed. The existing broader PSF expectation mismatch remains outside this change. Independent task-verifier verdict: **PARTIAL** — local behavior is verified, but migration application and production consumers are not yet verified.
- Deployment/push status: Commit/push pending at report creation. Apply the migration, then redeploy `extraction-worker` and `api`; dashboard redeployment is not required for backend-only changes.
- Known limitation/next action: Existing historical rows remain unchanged until explicitly backfilled. Verify migration/advisors and confirm new rows expose both model and pipeline confidence before starting the title/correction-layer phases.
