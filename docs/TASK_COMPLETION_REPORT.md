...
- Verification: Focused dedupe tests passed (`2 passed`); scoped `git diff --check` passed. Production SQL connectivity passed, `parsed_output_unified` was verified to expose `normalized_message` and `listing_index`, the partial expiry index was verified in `pg_indexes`, and `ANALYZE public.raw_messages` completed. Independent task-verifier verdict: **PARTIAL** — the live database repair is verified, but Coolify reports both resumed workers as `running:unknown` rather than a health-confirmed state.
- Deployment/push status: Commit `d1823eb1` is pushed to `origin/main`. The compatibility-view repair and queue index were applied in production using the Supabase Management API. Start requests were queued for `extraction-worker` and `semantic-embedding-worker`; no application redeploy was performed.
- Known limitations/next action: Confirm worker logs/health and monitor timeout/duplicate-claim rates after resume. Do not enable `security_invoker` on the public listing views without a replacement privacy architecture.

## 2026-09-12 — Read-only Supabase production audit

- Requested outcome: determine whether the Supabase schema, security model, queues, storage, and query workload are sound after the resize and repairs.
- Files/services changed: added `docs/SUPABASE_AUDIT_2026-09-12.md`; no production data or schema was changed by the audit.
- Findings: production migration ledger is 26 migrations behind the repository; 30,095 group messages remain unprocessed; extraction has 17,080 dead-lettered and 89,137 failed attempts; semantic embeddings have 24,314 HTTP 402 failures; client roles have 2,242 broad table-grant rows; an invalid index remains; vector/event/secret tables have disproportionate storage; and listing/progress queries remain slow.
- Positive checks: all inspected public tables had RLS enabled; typed listing rows had zero orphaned raw-message references and zero tenant mismatches; current worker heartbeats were present; the repaired view and expiry index were verified live.
...
## 2026-09-16 — Add agent plan, mission, and next actions

- Requested outcome: Make the agent visibly plan work, retain the current
  mission, and offer useful next actions instead of ending at a result table.
- Files/services changed: `frontend/src/app/chat/page.tsx` adds the current
  mission strip, live four-step work plan, and result follow-up actions;
  `frontend/src/app/globals.css` styles those states. The required Coolify
  service remains `propai-lab:main-app`.
...
- Deployment/push: Commit `ac2c6227` was pushed to `main`; the `mcp` service
  was not deployed in this task.
