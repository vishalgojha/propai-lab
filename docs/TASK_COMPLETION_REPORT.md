...
- Deployment/push: Supabase production change applied; no Coolify redeploy was triggered. The temporary key file `/home/vishal/supa.txt` was removed after use. This report was committed and pushed in `f577ef1c`.
...
## 2026-09-06 — Remove remaining building-name formatting artifacts

- Requested outcome: Remove WhatsApp Markdown wrappers from building names and aliases while preserving distinct building identities and source-backed inventory.
- Changes: Added production migrations `20260906190000_merge_formatted_building_identity_collisions.sql`, `20260906193000_normalize_remaining_building_alias_formatting.sql`, and `20260906194500_normalize_last_building_registry_name.sql`. Six rows with identical Google place identities were consolidated into their canonical registry rows; two distinct rows were retained and given readable disambiguators. Remaining aliases were normalized in place. No listing rows or raw/source message payloads were deleted; 26 redundant enrichment-source rows were removed only where an exact winner/provider/field duplicate already existed.
...
- Requested outcome: Preserve furnishing details such as “Semi Finished” from broker messages so rental similarity and listing cards can use them.
- Changes: `ai_extraction.py` now normalizes furnishing wording variants including `Semi Finished`, `Semi-Furnished`, `Part Furnished`, `S/F`, `SF`, `Fully Finished`, and `Builder Finished`. It also checks the model provenance field when the top-level furnishing field is omitted, while raw-source grounding remains mandatory. `architecture.md` records the shared normalization rule. Added regression tests in `tests/test_ai_extraction_field_fallbacks.py`.
- Verification: Focused tests passed (`19 passed`); scoped `git diff --check` passed. Live Supabase audit found 48 rental rows with non-empty furnishing provenance but blank typed furnishing; this is a mixed candidate set because the provenance field also contains unrelated notes such as “Modular kitchen & ACs,” so no unsafe bulk repair was applied. The independent task-verifier second pass returned PARTIAL for production completion: the code path passes, but deployment and evidence-based historical repair remain pending.
...
- Deployment/push: Scoped commit `7832dcf0` was committed and pushed to `redesign/propai-product-interface`. Coolify service `propai-lab:main-app` needs redeployment for `app.propai.live`; no production data or service was changed.
...
- Deployment/push: Chat-only commit `6e622832` was pushed to `redesign/propai-product-interface`. Coolify service `propai-lab:main-app` needs redeployment for `app.propai.live`; no production data or service was changed.
