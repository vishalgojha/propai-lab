...
def enrich_location(storage: "SqliteStorage", d: dict) -> None:
...
def _check_group_location_context(storage: "SqliteStorage", parsed_id: int,
...
def _check_substring_aliases(storage: "SqliteStorage", raw_text: str, parsed_id: int) -> bool:
    search = raw_text.lower()
    rows = storage.db.execute(
