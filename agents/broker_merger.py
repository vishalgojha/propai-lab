...
def check_for_broker_merge(storage: "SqliteStorage", broker_id: int | None = None) -> None:
    if broker_id:
...
def _check_single_broker(storage: "SqliteStorage", broker_id: int) -> None:
...
def _scan_all_brokers(storage: "SqliteStorage") -> None:
...
def _find_brokers_by_jid(storage: "SqliteStorage", name: str) -> set[int]:
...
def _create_merge_suggestion(storage: "SqliteStorage", keeper: dict, dup: dict) -> None:
