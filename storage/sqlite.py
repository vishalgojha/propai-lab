...
from .base import (
    ...
    Evaluation, SyncJob, SyncCheckpoint, AISuggestion, LLMProvider,
    dict_to_dataclass,
)
...
class SqliteStorage(Storage):
    ...
    def init_schema(self):
        ...
        migs = [
            ...
            "CREATE TABLE IF NOT EXISTS llm_providers (id INTEGER PRIMARY KEY AUTOINCREMENT, provider_name TEXT NOT NULL, provider_type TEXT NOT NULL DEFAULT 'openai', api_key TEXT NOT NULL DEFAULT '', base_url TEXT NOT NULL DEFAULT '', model_name TEXT NOT NULL DEFAULT '', is_active INTEGER NOT NULL DEFAULT 0, created_at TEXT DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')), updated_at TEXT DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')))",
        ]
        for sql in migs:
            try:
                self.db.execute(sql)
        ...
        # Historical MESSAGES_SET events usually store sender id at $.data.sender.id.
        try:
            self.db.execute("""
                UPDATE raw_messages
    ...
    def _source_group_maps(self) -> tuple[dict[str, str], dict[str, str]]:
        rows = self.db.execute(
            "SELECT group_id, group_name FROM source_sync_jobs WHERE group_id IS NOT NULL AND group_id != ''"
        ).fetchall()
    ...
    def _decorate_chat_row(self, row: dict, chat_id: str, chat_type: str, chat_name: str, count: int | None = None) -> dict:
        decorated = dict(row)
    ...
    def get_chats(self, limit: int = 500, offset: int = 0, tenant_id: str | None = None) -> list[dict]:
        ...
        rows = self.db.execute(
            """
            SELECT * FROM raw_messages
            WHERE COALESCE(group_name, '') NOT LIKE '%@newsletter'
              AND COALESCE(sender_jid, '') NOT LIKE '%@newsletter'
              AND COALESCE(group_name, '') NOT IN ('status@broadcast', 'broadcast')
              AND COALESCE(group_name, '') NOT LIKE '%@broadcast'
        ...
        for chat_id, latest in grouped.items():
            ...
            chats.append(latest)
        chats.sort(key=lambda r: (r.get("timestamp") or r.get("created_at") or "", r.get("id") or 0), reverse=True)
    ...
    def get_chat_messages(self, chat_id: str, limit: int = 200, offset: int = 0, tenant_id: str | None = None) -> list[RawMessage]:
        ...
        clauses = ["message_uid LIKE ?"]
        params: list = [f"{chat_id}:%"]
        if names:
            clauses.append(f"group_name IN ({','.join('?' for _ in names)})")
            params.extend(names)
        clauses.append("sender_jid = ?")
        params.append(chat_id)
        if digits:
            clauses.append("sender_phone = ?")
...
class ClientStorage:
    ...
    # ── LLM Providers (ClientStorage) ──────────────────────────

    def get_llm_providers(self) -> list[LLMProvider]:
        rows = self.db.execute(
            "SELECT * FROM llm_providers ORDER BY is_active DESC, provider_name ASC"
        ).fetchall()
        return [dict_to_dataclass(LLMProvider, r) for r in rows]

    def get_active_llm_provider(self) -> Optional[LLMProvider]:
        row = self.db.execute(
            "SELECT * FROM llm_providers WHERE is_active = 1 LIMIT 1"
        ).fetchone()
        return dict_to_dataclass(LLMProvider, row) if row else None
    ...
    def save_llm_provider(self, provider: LLMProvider) -> int:
        if provider.id:
            ...
            if existing and (not provider.api_key or "****" in provider.api_key):
                provider.api_key = existing["api_key"]
            if provider.is_active:
                self.db.execute(
                    "UPDATE llm_providers SET is_active = 0 WHERE id != ?", (provider.id,)
                )
            self.db.execute(
                """UPDATE llm_providers SET provider_name = ?, provider_type = ?,
                   api_key = ?, base_url = ?, model_name = ?, is_active = ?,
                   updated_at = strftime('%Y-%m-%dT%H:%M:%SZ', 'now')
                   WHERE id = ?""",
            ...
            if provider.is_active:
                self.db.execute("UPDATE llm_providers SET is_active = 0")
            cur = self.db.execute(
                """INSERT INTO llm_providers (provider_name, provider_type, api_key,
