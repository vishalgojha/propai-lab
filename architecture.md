...
| Textual broker identity fields (`broker_name`, `broker_company`, `broker_rera_number`) must have compact-token overlap with the item-scoped source slice before typed persistence; unsupported values are cleared, review-flagged, and write-blocked. Sender phone/JID and directory linkage remain provenance metadata. | Broker metadata can otherwise become a fabricated message fact and contaminate outreach/contact matching; the shared extraction/persistence gate keeps it out of new typed rows while preserving audit evidence. |
