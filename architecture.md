...
| open | Auto Matched production end-to-end behavior still needs a real save → match → UI verification after deploy. The live matcher had been running the API/Uvicorn image command, and its first worker run exposed typed per-table IDs being written into legacy-FK `requirement_matches`. | Matcher has a dedicated `Dockerfile.matcher` with an immutable worker CMD. Internal `*_unified_matching` projections now expose globally keyed `legacy_source_id` values without changing public typed IDs; deploy and verify a saved match before closing. | `Dockerfile.matcher`, `matching/worker.py`, `matching/service.py`, `supabase/migrations/20260902230000_matching_legacy_id_projection.sql`, `docs/PROPai_FRESH_OPERATIONAL_AUDIT_2026-09-02.md` |
