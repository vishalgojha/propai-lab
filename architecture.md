...
| open | Auto Matched production end-to-end behavior still needs a real save → match → UI verification after deploy. The live matcher had been running the API/Uvicorn image command despite a worker start-command setting. | Matcher now has a dedicated `Dockerfile.matcher` with an immutable worker CMD; deploy it, verify `[match-worker] started`, heartbeat, queue progress, and a saved match before closing. | `Dockerfile.matcher`, `matching/worker.py`, `matching/service.py`, `docs/PROPai_FRESH_OPERATIONAL_AUDIT_2026-09-02.md` |
